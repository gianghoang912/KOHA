...
sub AddShelf {
    ...
    my $query = qq(INSERT INTO virtualshelves
        (shelfname,owner,category,sortfield,allow_add,allow_delete_own,allow_delete_other,img)
        VALUES (?,?,?,?,?,?,?,?));
    ...
    $sth->execute(
		...
		$hashref->{img} || "default/bosuutap.jpg",
		);
    return if $sth->err;
...
sub ModShelf {
	...
	if($hashref->{img}){
		 $query= "UPDATE virtualshelves SET shelfname=?, category=?, sortfield=?, allow_add=?, allow_delete_own=?, allow_delete_other=?, img=? WHERE shelfnumber=?";
		$sth = $dbh->prepare($query);
		$sth->execute(
			$hashref->{shelfname}//$oldrecord->{shelfname},
			$hashref->{category}//$oldrecord->{category},
