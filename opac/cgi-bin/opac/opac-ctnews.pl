...
my $idnew = $input->param("idnew");
my $idgroup = $input->param("idgroup");
my ( $template, $borrowernumber, $cookie ) = get_template_and_user(
    {
        template_name   => "opac-ctnews.tt",
        type            => "opac",
        query           => $input,
...
my $all_koha_news   = &getTinTuc($news_lang,$homebranch,$idgroup,10);
...
my $cttin = get_opac_new($idnew);

$template->{VARS}->{'cttin'} = $cttin;


my $view_VirtualShelves = &ViewVirtualShelves();
my $VirtualShelves_count = scalar @$view_VirtualShelves;
$template->param(
		bosuutap => $view_VirtualShelves,
		tong_bst => $VirtualShelves_count,
);
...
#phan lien quan den bo cai dat
my $bocaidat = &getTinTuc($news_lang,$homebranch,8,5);
my $tailieukhac = &getTinTuc($news_lang,$homebranch,9,5);
my $hdcaidat = &getTinTuc($news_lang,$homebranch,10,5);
my $hdsudung = &getTinTuc($news_lang,$homebranch,11,5);

$template->param(
	bocaidats => $bocaidat,
	tailieukhacs => $tailieukhac,
	hdcaidats => $hdcaidat,
	hdsudungs => $hdsudung
);
