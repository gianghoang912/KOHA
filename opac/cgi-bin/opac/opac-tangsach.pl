...
if ($negcaptcha ) {
    print $input->redirect("/cgi-bin/koha/opac-tangsach.pl");
    exit;
}
...
if ( C4::Context->preference("AnonSuggestions") ) {
    ( $template, $borrowernumber, $cookie ) = get_template_and_user(
        {
            template_name   => "opac-tangsach.tt",
            query           => $input,
            type            => "opac",
...
if ( $op eq "add_confirm" ) {
	...
	$op              = 'else';
    print $input->redirect("/cgi-bin/koha/opac-tangsach.pl?op=else&submitted=1");
    exit;
}
...
if ( $op eq "delete_confirm" ) {
    ...
    $op = 'else';
    print $input->redirect("/cgi-bin/koha/opac-tangsach.pl?op=else&deleted=1");
    exit;
}
